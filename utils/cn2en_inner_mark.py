...
def translate_etching_stats(match, item, target):
    ...
    if not attribute_level:
        ...
        target.setdefault("etching_stats", {})
        target["etching_stats"].setdefault("level", {})

        if attribute_pairs:
            if value:
                target["etching_stats"]["level"][attribute_level] = dict(
                    zip(attribute_pairs, value.split(","))
                )
            else:
                target["etching_stats"]["level"][attribute_level] = {}


def translate_crossing_stats(match, item, target):
    key, value = item
    attribute_num = match.group(1)

    target.setdefault("crossing_stats", {})
    target["crossing_stats"].setdefault(attribute_num, {})

    if match.group(2) == "解锁等级":
        attribute_type = "unlock_level"
        target["crossing_stats"][attribute_num][attribute_type] = value
    elif match.group(2):
        target["crossing_stats"][attribute_num].setdefault("level", {})
        target["crossing_stats"][attribute_num]["level"][match.group(2).lower()] = value
    else:
        attribute_type = "name"
        target["crossing_stats"][attribute_num][attribute_type] = value
...
def translate_mark_awaken(match, item, target):
    key, value = item
    awaken_stage = 2 if match.group(1) == "II" else 4
    awaken_option = 0 if match.group(2) == "A" else 1
    target.setdefault("mark_awaken", {})
    target["mark_awaken"].setdefault(awaken_stage, [[], []])
    target["mark_awaken"][awaken_stage][awaken_option].append(value)
...
def translate_inner_mark(origin_dict):
    ...
    for key, value in origin_dict.items():
        ...
        if match := re.match(r"^属性(\d+)(解锁等级|LV\d*)?", key):
            translate_crossing_stats(match, item, translated_dict)
        elif match := re.match(r"^基础属性([Ll][Vv]\d+)?", key):
            translate_etching_stats(match, item, translated_dict)
            ...
            translate_mark_awaken(match, item, translated_dict)

        elif english_attribute := base_dict.get(key, key):
            valid_attributes = {"acquisition", "regular_skill", "expression", "tag"}

            if english_attribute in valid_attributes:
                translated_dict[english_attribute] = value.split(",")
            elif english_attribute == "attribute":
                translated_dict[english_attribute] = attribute_dict[value]
            else:
